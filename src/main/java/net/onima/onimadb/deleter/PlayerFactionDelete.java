...
public class PlayerFactionDelete extends MongoAccessor {
	...
	public PlayerFactionDelete(PlayerFaction faction, VoidCallback<MongoResult> callback) {
		super(callback);
	...
	public void execute() throws MongoException {
		((MongoDeleteResult) result).setDocumentsDeleted(OnimaMongo.get(OnimaCollection.PLAYER_FACTIONS).deleteOne(Filters.eq("uuid", faction.getUUID().toString())).getDeletedCount());
